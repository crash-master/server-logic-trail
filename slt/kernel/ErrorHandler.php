...
class ErrorHandler{
	...
	public function __construct(){
		...
		$this -> clear_log_after_days = Config::get() -> system -> ErrorHandler -> LogStorageLifeInDays;
		if($err_disp){
			error_reporting(-1);
		}else{
			error_reporting(0);
	...
	public function fatalErrorHandler(){
		...
		if ($error){
			if($error['type'] == E_ERROR || $error['type'] == E_PARSE || $error['type'] == E_COMPILE_ERROR || $error['type'] == E_CORE_ERROR){
				Events::register('error_was_found', $error);
	...
	public function handler($errno, $errstr, $errfile, $errline){
		...
		Events::register('error_was_found', [
			'type' => $err_type,
			'message' => $errstr,
			'file' => $errfile, 
			'line' => $errline
	...
	protected function log($errtype, $errno, $errstr, $errfile, $errline, $endflag = false){
		...
		// dd($this -> path_to_log_dir . '/errors_' . $timestamp . '.log');
		if(!file_exists($this -> path_to_log_dir)){
			mkdir($this -> path_to_log_dir, 0755);
		}
		$filename = $this -> path_to_log_dir . '/errors_' . $timestamp . '.log';
	...
	public function viewErrs(){
		...
		foreach($this -> errs_src as $err){
			...
			$page = view('slt/error-page-dev', compact('errno', 'err_type', 'errstr', 'errfile', 'errline', 'code'));
			$errs[] = $page;
		}

		show(view('slt/errors', ['errs' => $errs]));
	}
	...
	public function viewFatalError($errno, $errstr, $errfile, $errline){
		...
		?>
		<style>
			*{padding:0;margin:0}body{background:#444}.slt-err-block{position:absolute;z-index:10000;top:0;left:0;width:100%;height:auto;padding:20px;background-color:#444;color:#eee;font-family:Courier!important;font-size:15px}.slt-err-block .line{display:block;padding:10px;background:#eee;font-weight:700}.slt-err-block .line.error{background-color:#ccc;color:#B71C1C}.slt-err-block code{padding:10px;color:#00695C}.slt-err-block code .line b{padding-right:10px;border-right:2px solid #ccc;display:inline-block;margin-right:10px;font-weight:400}
		</style>
